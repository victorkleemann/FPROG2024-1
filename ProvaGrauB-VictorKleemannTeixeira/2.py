...
def gerarToupeiras(): #Função para gerar as toupeiras em posições aleatórias
    arrayTouperiras = [['-' for i in range(4)] for i in range(4)] # Matriz cheia de posições vazias
    for o in range(4):
        while True:
            x = random.randint(0,3) # seleciona uma linha aleatória
            y = random.randint(0,3) # seleciona uma coluna aleatoria
            if arrayTouperiras [x][y] == '-':  
                arrayTouperiras[x][y] = 'T'
                break
    return arrayTouperiras #Retorna a array com as toupeiras posicionadas
